...
import (
	...
	"github.com/imashen/CloudflareSpeedTest/task"
	"github.com/imashen/CloudflareSpeedTest/utils"
)
...
https://github.com/imashen/CloudflareSpeedTest
...
参数：
    ...
    -url https://download.13140521.xyz/speedtest
...
`
	...
	flag.StringVar(&task.URL, "url", "https://download.13140521.xyz/speedtest", "指定测速地址")
	...
	if printVersion {
		...
		checkUpdate()
		if versionNew != "" {
			fmt.Printf("*** 发现新版本 [%s]！请前往 [https://github.com/imashen/CloudflareSpeedTest] 更新！ ***", versionNew)
		} else {
...
func main() {
	...
	fmt.Printf("# imashen/CloudflareSpeedTest %s \n\n", version)
	...
	if versionNew != "" {
		fmt.Printf("\n*** 发现新版本 [%s]！请前往 [https://github.com/imashen/CloudflareSpeedTest] 更新！ ***\n", versionNew)
	}
	endPrint()
}
...
func checkUpdate() {
	...
	res, err := client.Get("https://api.imashen.cn/ver/cloudflarespeedtest.txt")
	if err != nil {
		return
	}
	// 读取资源数据 body: []byte
